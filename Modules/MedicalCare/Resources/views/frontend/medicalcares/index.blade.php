        ...
        <div class="container mx-auto px-4">
            <div class="max-w-3xl">
                <div class="inline-block bg-white/20 backdrop-blur-sm rounded-full px-4 py-2 mb-4">
                    <span class="text-white text-sm font-medium">🏥 @lang('Healthcare Services in Yogyakarta')</span>
                </div>
                <h1 class="text-4xl md:text-6xl font-bold text-white mb-4 leading-tight">
                    Medical
                    <span class="block text-blue-200">Care Hub</span>
                </h1>
                <p class="text-lg text-gray-200 mb-6 max-w-2xl">
                    @lang('Discover trusted healthcare facilities in Yogyakarta with an easy search system and comprehensive information.')
                </p>
                
                <!-- Quick Stats -->
                <div class="flex flex-wrap gap-4 mb-6">
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                        <div class="text-xl font-bold text-white">{{ $medicalcares->total() }}</div>
                        <div class="text-blue-200 text-sm">@lang('Total Facilities')</div>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
                        <div class="text-xl font-bold text-white">3</div>
                        <div class="text-blue-200 text-sm">@lang('Service Categories')</div>
                    </div>
                    <div class="bg-white/10 backdrop-blur-sm rounded-lg px-4 py-2">
        ...
        <div class="max-w-6xl mx-auto">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
                <h3 class="text-2xl font-bold text-gray-800 dark:text-white mb-6 text-center">
                    <i class="fas fa-search text-blue-600 mr-2"></i>Find Your Perfect Healthcare Facility
                </h3>
                
                <form action="{{ route('frontend.medicalcares.index') }}" method="GET" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <!-- Search Input -->
                        <div class="lg:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Search Facilities</label>
                            <div class="relative">
                                <input type="text" 
                                       name="search" 
                        ...
                        <!-- Type Filter -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Care Type</label>
                    ...
                    <div class="flex justify-center gap-4">
                        <button type="submit" class="bg-blue-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-blue-700 transition duration-200">
                            <i class="fas fa-search mr-2"></i>Search
                        </button>
                        <a href="{{ route('frontend.medicalcares.index') }}" class="px-8 py-3 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg font-semibold hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-200">
...
<section id="care-facilities" class="bg-gray-50 dark:bg-gray-900 min-h-screen py-12">
    <div class="container mx-auto px-4">
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-80 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 sticky top-6">
                    ...
                    <form action="{{ route('frontend.medicalcares.index') }}" method="GET" class="space-y-4" id="filterForm">
                        <!-- Search Input -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">@lang('Search Facilities')</label>
                            <div class="relative">
                                <input type="text" name="search" value="{{ request('search') }}" 
                                       placeholder="@lang('Facility name, location...')" 
                                       class="w-full pl-10 pr-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white dark:bg-gray-700 dark:text-white">
                        ...
                        <!-- Type Filter -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">@lang('Service Type')</label>
                            <div class="space-y-2">
                                @php
                                $types = [
                                    ...
                                    ['value' => 'Primary Health care', 'label' => __('Primary Care'), 'color' => 'teal', 'icon' => 'M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zm0 4a1 1 0 011-1h12a1 1 0 011 1v8a1 1 0 01-1 1H4a1 1 0 01-1-1V8z'],
                                    ['value' => 'Secondary Health care', 'label' => __('Secondary Care'), 'color' => 'cyan', 'icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
                                    ['value' => 'Tertiary Health care', 'label' => __('Tertiary Care'), 'color' => 'blue', 'icon' => 'M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z']
                                ];
                                @endphp
                        ...
                        <div>
                            ...
                            <select name="sort" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent bg-white dark:bg-gray-700 dark:text-white">
                        ...
                        <div class="space-y-3 pt-4">
                            <button type="submit" class="w-full bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-white font-semibold py-3 px-4 rounded-lg transition-all duration-200">
                                <svg class="w-5 h-5 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                                </svg>
                                @lang('Apply Filters')
                            </button>
                            <a href="{{ route('frontend.medicalcares.index') }}" class="w-full bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 font-medium py-3 px-4 rounded-lg transition-all duration-200 text-center block">
                                @lang('Reset Filters')
                            </a>
                        </div>
                    </form>
                ...
                <!-- Quick Info -->
                <div class="bg-gradient-to-br from-teal-600 to-cyan-600 rounded-xl p-6 text-white">
                    <h4 class="font-bold mb-2">💡 @lang('Search Tips')</h4>
                    <ul class="text-sm space-y-1 text-teal-100">
                        <li>• @lang('Use specific keywords')</li>
                        <li>• @lang('Filter by service type')</li>
                        <li>• @lang('Check detailed info on each card')</li>
                    </ul>
                </div>
            </div>
            ...
            <div class="flex-1">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                            @lang('Healthcare Facilities')
                        </h2>
                        <p class="text-gray-600 dark:text-gray-400">
                            @lang('Found') {{ $medicalcares->total() }} @lang('facilities')
                ...
                @if($medicalcares->count() > 0)
                    ...
                    <div id="masonryView" class="masonry-container">
                        @foreach ($medicalcares as $index => $medicalcare)
                        @php
                        $details_url = route("frontend.medicalcares.show", [encode_id($medicalcare->id), $medicalcare->slug]);
                        $colors = [
                            'Primary Health care' => ['bg' => 'from-teal-500 to-teal-600', 'text' => 'text-teal-600', 'light' => 'bg-teal-50', 'dark' => 'bg-teal-900/20'],
                            'Secondary Health care' => ['bg' => 'from-cyan-500 to-cyan-600', 'text' => 'text-cyan-600', 'light' => 'bg-cyan-50', 'dark' => 'bg-cyan-900/20'],
                            'Tertiary Health care' => ['bg' => 'from-blue-500 to-blue-600', 'text' => 'text-blue-600', 'light' => 'bg-blue-50', 'dark' => 'bg-blue-900/20']
                        ];
                        $color = $colors[$medicalcare->type] ?? ['bg' => 'from-gray-500 to-gray-600', 'text' => 'text-gray-600', 'light' => 'bg-gray-50', 'dark' => 'bg-gray-900/20'];
                    ...
                    <div id="listView" class="space-y-6 hidden">
                        ...
                        $details_url = route("frontend.medicalcares.show", [encode_id($medicalcare->id), $medicalcare->slug]);
                        $colors = [
                            'Primary Health care' => ['bg' => 'from-teal-500 to-teal-600', 'text' => 'text-teal-600'],
                            'Secondary Health care' => ['bg' => 'from-cyan-500 to-cyan-600', 'text' => 'text-cyan-600'],
                            'Tertiary Health care' => ['bg' => 'from-blue-500 to-blue-600', 'text' => 'text-blue-600']
                        ];
                        $color = $colors[$medicalcare->type] ?? ['bg' => 'from-gray-500 to-gray-600', 'text' => 'text-gray-600'];
                        @endphp
                        ...
                        <div class="max-w-md mx-auto">
                            <div class="w-24 h-24 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center mx-auto mb-6">
                                <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.569.58-6.593 1.609M12 3a9 9 0 100 18 9 9 0 000-18z"/>
                                </svg>
                            </div>
                            <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">@lang('No Facilities Found')</h3>
                            <p class="text-gray-600 dark:text-gray-400 mb-6">
                                @lang('Sorry, no healthcare facilities match your search criteria.')
                            </p>
                            <a href="{{ route('frontend.medicalcares.index') }}" 
                               class="inline-flex items-center bg-teal-600 hover:bg-teal-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.toggleView = function(viewType) {
        ...
        if (viewType === 'masonry') {
            ...
            // Reinitialize masonry after showing
    ...
    if (urlParams.get('search') || urlParams.get('type') || urlParams.get('sort')) {
        setTimeout(() => {
    ...
    document.querySelectorAll('.masonry-item, .view-btn').forEach(element => {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            // Add visual feedback
            if (value.length > 0) {
                this.parentNode.classList.add('border-teal-500');
            } else {
                this.parentNode.classList.remove('border-teal-500');
            }
